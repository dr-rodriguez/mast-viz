...
from cdshealpix import polygon_search, to_ring
...
def get_polygon_cdshealpix(ra_list, dec_list, depth=8):
    # Use the CDSHEALPIX Rust package to get all HEALPix ids for the specified depth
    # within the polygon described by ra/dec lists. Also convert to RING indexing
    lon = u.Quantity(ra_list * u.deg)
    lat = u.Quantity(dec_list * u.deg)
    nested, _, fully_covered = polygon_search(lon, lat, depth, flat=True)
    ring = to_ring(nested, depth)
    return ring
